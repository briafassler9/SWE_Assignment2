...
exports.findAllSolutions = function(grid, dictionary) {
  let solutions = [];
   ...
   // Setup all data structures (i.e visited, solutions, dictionary)
   
   let solutionSet = new Set();
   let hash = createHashMap(dictionary);
   
   //Iterate over all the NXN grid - find all words that begin with the grid[y][x]
   for (let y = 0; y < N; y++) {
     for (let x = 0; x < N; x++) {
       let word = "";
...
findWords = function(word, y, x, grid, visited, hash, solutionSet) {
  ...
  if (isPrefixOrWord(word, hash)) {
    // 1a. Is that prefix an actual word in the dictionary, mark as visited
        visited[y][x] = true;
        if (isWord(word, hash)) {
